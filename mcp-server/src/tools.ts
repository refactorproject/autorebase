...
${result.changelog ? `
### Summary
- **Timestamp**: ${result.changelog.timestamp}
- **Files Processed**: ${result.changelog.files_processed.length}
- **Patches Generated**: ${result.changelog.patches_generated.length}
- **Patches Applied**: ${result.changelog.patches_applied.length}
- **F1 Files Created**: ${result.changelog.f1_files_created.length}
- **Backup Files**: ${result.changelog.backup_files.length}
- **Reject Files**: ${result.changelog.reject_files.length}
- **Three-Way Merges**: ${result.changelog.three_way_merges.length}

### Files Processed
${result.changelog.files_processed.map((file: string) => `- ${file}`).join('\n')}

### Successful Patches Applied
${result.changelog.patches_applied.map((patch: any) => 
  `- **${patch.file}** (${patch.patch_type}) - ${patch.timestamp}`
).join('\n')}

### AI Resolutions
${result.changelog.three_way_merges.filter((merge: any) => merge.status === 'success').map((merge: any) => 
...
📁 **Changelog saved to**: \`${result.changelog_path}\`

### Complete Changelog JSON
\`\`\`json
